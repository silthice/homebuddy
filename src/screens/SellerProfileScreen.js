...
import { Container, Header, Title, Drawer, Button, Icon, Left, Right, Body, Item, Input, Card, CardItem, Footer, FooterTab, Content, Thumbnail, Tab, Tabs, ScrollableTab  } from "native-base";
...
class SellerProfileScreen extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        this.state.sellerID = this.props.route.params.seller_id;
        console.log('seller id ', this.state.sellerID);
        this.initInfo();
    }
    ...
    getSellerProductListing = async () => {

        let seller_id = this.state.sellerID;

        let result = await new HTTP().post(CONSTS.clone_URL + 'searchBySellerJs', {seller_id});
    ...
    getFollowingList = async() =>{
        ...
        let result = await new HTTP().post(CONSTS.clone_URL + 'followingListJs', {mlm_id, session_no});

        if (result.status){
            this.setState({ myFollowingLst: result.list }, () => {
        ...
        console.log('Seller Profile Src getFollowingList: ', result.status);
    }

    followSeller(target_user_id){
        console.log('followseller', target_user_id);
                ...
                <View style={{marginLeft: 10, marginRight: 10}}>
                    <View style={{flexDirection: 'row', alignItems: 'center'}}>
                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                            ...
                            {
                            this.state.chkSellerID != this.state.sellerID?

                                <View style={{borderRadius: 5, backgroundColor: '#FF7A59', padding: 8, position: 'absolute', right: 10, width: 80, alignItems: 'center'}}>
                                {
                                    this.state.followed == true?
                                        <Text style={{fontWeight: 'bold', fontSize: 12, color: 'white'}}>Unfollow</Text>
                                        :
                                        <TouchableOpacity onPress={ () => this.followSeller(this.state.sellerID)}>
                                            <Text style={{fontWeight: 'bold', fontSize: 12, color: 'white'}}>Follow</Text> 
                                       </TouchableOpacity>
                                }
